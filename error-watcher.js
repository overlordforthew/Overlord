...
const observerGuard = createObserverGuard({ dataDir: process.env.DATA_DIR || './data' });
...
export function watchDockerEvents() {
  ...
  dockerWatcherProc.stdout.on('data', (data) => {
    dockerWatcherBuffer += data.toString();
    const lines = dockerWatcherBuffer.split('\n');
    dockerWatcherBuffer = lines.pop() || '';
    for (const line of lines) {
      ...
      const [, container, event, exitCodeText = '0'] = match;
      ...
      setTimeout(async () => {
        ...
        try {
          const signal = await observerGuard.trackSignal({
            key: `docker:${normalizeName(container)}:${event}:${exitCode}`,
            minHits: 1,
            corroborated: true,
            cooldownMs: COOLDOWN_MS,
            meta: { container, event, exitCode },
          });
          if (!signal.shouldEscalate) return;
          ...
          await createRepairTask({
            ...
            riskLevel: 'low',
            executor: 'container',
            dedupeKey: `docker:${normalizeName(container)}`,
            escalation: 'whatsapp_first',
            successCriteria: `Container ${container} running and healthy`,
            nextAction,
            verifier: {
              type: 'command',
              command: `docker inspect --format='{{.State.Running}} {{if .State.Health}}{{.State.Health.Status}}{{else}}no-healthcheck{{end}}' "${container}" 2>/dev/null`,
              expectRegex: 'true',
              failureMessage: `Container ${container} is not running`,
            },
            source: 'observer',
          });
        } catch (err) {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (count > 5) {
        ...
        const signal = await observerGuard.trackSignal({
          key: `traefik:${backend}:${status}`,
          minHits: priority === 'urgent' ? 1 : 2,
          cooldownMs: COOLDOWN_MS,
          meta: { backend, status, count },
        });
        if (!signal.shouldEscalate) continue;
        setCooldown(key);
        ...
        await createRepairTask({
          ...
          riskLevel: 'low',
          executor: 'container',
          dedupeKey: `traefik:${backend}`,
          escalation: 'whatsapp_first',
          successCriteria: `${backend} returning 200 OK`,
          nextAction,
          verifier: {
            type: 'command',
            command: `docker ps --format "{{.Names}} {{.Status}}" 2>/dev/null | grep -i "^${backend} "`,
            expectRegex: 'Up',
            failureMessage: `Backend ${backend} is not reporting as Up`,
          },
          source: 'observer',
        });
      }
