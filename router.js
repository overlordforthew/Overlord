...
Reply with ONLY one word: COMPLEX, MEDIUM, or SIMPLE`;
  ...
  try {
    const classificationResult = await runStatelessIntelligence({
      systemPrompt: 'You are a message classifier. Reply with only one word: COMPLEX, MEDIUM, or SIMPLE.',
...
export async function routeMessage(parsed, opts = {}) {
  const requestedMode = opts.mode || process.env.ROUTER_MODE || 'alpha';
  const mode = getIntelligenceBackend() === 'codex' ? 'alpha' : requestedMode;
...
export function routeTriage(mode) {
  const requestedMode = mode || process.env.ROUTER_MODE || 'alpha';
  const currentMode = getIntelligenceBackend() === 'codex' ? 'alpha' : requestedMode;
