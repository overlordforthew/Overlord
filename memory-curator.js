/**
 * Memory Curator v3 — backend-configurable extraction with vector dedup and crash safety.
 *
 * Changes from v2:
 * - Uses the configured intelligence backend for durable fact extraction
...
Extract new or updated facts. Return {"episodic":[],"semantic":[]} if nothing new.`;
    ...
    // Use the configured intelligence backend for extraction quality.
    let result;
    try {
      result = await callExtractor(EXTRACT_SYSTEM, prompt);
    } catch (err) {
      console.error('[memory-curator] Extraction failed:', err.message);
      return 0;
    }
    ...
    if (stored > 0) {
    console.log(`[memory-curator] Extracted ${stored} facts from ${jid}`);
    }
