...
export function startServer(sockRef, sendResponse, connectionHealth) {
  ...
  app.post('/api/send', requireToken, async (req, res) => {
    try {
      ...
      // Support audio file path: {"to":"...", "audio":"/tmp/file.mp3"}
      if (req.body.audio) {
        const audioPath = path.resolve(req.body.audio);
        const ALLOWED_AUDIO_DIRS = ['/tmp/', '/app/data/', '/app/media/'];
        if (audioPath.includes('..') || !ALLOWED_AUDIO_DIRS.some(d => audioPath.startsWith(d))) {
          return res.status(400).json({ error: 'Audio path not allowed' });
        }
        const audioBuf = fs.readFileSync(audioPath);
        await sockRef.sock.sendMessage(jid, {
          audio: audioBuf,
          mimetype: req.body.audio.endsWith('.wav') ? 'audio/wav' : 'audio/mpeg',
          ptt: false,
        });
        if (text) await sendResponse(sockRef.sock, jid, text);
      ...
      console.error('[api] Error:', err.message); res.status(500).json({ error: 'Internal server error' });
    }
  });
  ...
  // POST /api/customer-signal — record customer evidence from any project (token-protected)
  app.post('/api/customer-signal', requireToken, async (req, res) => {
    try {
      const signal = recordSignal(req.body);
  ...
  function summarizeMcAuthPoolMessage(message) {
  ...
  function isTransientMcAuthPoolMessage(message) {
    return /terminating connection due to administrator command|Connection terminated unexpectedly|the database system is shutting down|the database system is starting up/i.test(message);
  }

  mcPool.on('error', (err) => {
    const summary = summarizeMcAuthPoolMessage(err.message);
    if (isTransientMcAuthPoolMessage(summary)) {
      console.warn(`[mc-auth] Pool connection reset during DB restart: ${summary}`);
      return;
    }
    console.error('[mc-auth] Pool issue:', summary);
  });
    ...
    try {
      ...
      if (result.rows.length > 0) {
        ...
        if (mcMailer) {
          ...
          console.log(`[mc-auth] Password reset requested but no SMTP configured (token not logged for security)`);
        }
      }
...
- Never share server details, API keys, or internal infrastructure info`;
  ...
  app.post('/api/contact', contactRateLimit, async (req, res) => {
    try {
      ...
      // Record customer signal for Overlord's business brain
      try {
        recordSignal({
          project: routing.label === 'Master&Commander' ? 'MasterCommander' : routing.label === 'NamiBarden' ? null : routing.label,
          type: 'contact',
          email: email.trim(),
          name: name.trim(),
          content: message.trim().substring(0, 300),
          pain: (subject || message).trim().substring(0, 100),
          source: 'contact_form',
          metadata: { origin, plan: plan || null },
        });
      } catch { /* non-critical */ }
