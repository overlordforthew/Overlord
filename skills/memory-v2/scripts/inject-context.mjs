...
function formatBriefing(briefing, project) {
  ...
  // Autonomous activity (patrol findings + proposals)
  try {
    ...
    if (patrolAge < 24 * 60 * 60 * 1000 && patrol.totalFindings > 0) {
      lines.push(`AUTONOMOUS ACTIVITY (last patrol):`);
      for (const f of (patrol.autoFixed || []).slice(0, 3)) {
        lines.push(`  ✅ [auto-fixed] ${f.project || 'system'}: ${f.detail}`);
      }
      for (const f of (patrol.proposed || []).slice(0, 3)) {
        lines.push(`  📋 [proposal #${f.proposalId}] ${f.project || 'system'}: ${f.detail}`);
      }
      lines.push(``);
    }
  } catch { /* no patrol data yet */ }

  // Pending proposals
  try {
    const proposalsRaw = readFileSync('/root/overlord/data/proposals.json', 'utf8');
    const proposals = JSON.parse(proposalsRaw).filter(p =>
      p.status === 'pending' && new Date(p.expiresAt).getTime() > Date.now()
    );
    if (proposals.length > 0) {
      lines.push(`PENDING PROPOSALS (${proposals.length} awaiting response on WhatsApp):`);
      for (const p of proposals.slice(0, 5)) {
        const risk = { low: '🟢', medium: '🟡', high: '🔴' }[p.risk] || '🟡';
        lines.push(`  ${risk} #${p.id} ${p.title}${p.project ? ` [${p.project}]` : ''}`);
      }
      lines.push(``);
    }
  ...
  lines.push(``);
  lines.push(`PROACTIVE MODE: After answering, surface 1-2 related observations from your monitoring. Push back on bad ideas. Suggest what Gil should do next based on what you've been seeing. You're not waiting for orders — you're the one who's been watching the ship while he was away.`);
...
async function main() {
  ...
  try {
    // Mode toggle: OVERLORD_MODE=neutral skips personality injection
    const mode = process.env.OVERLORD_MODE || 'overlord';
    if (mode === 'neutral') {
      process.stdout.write('{}');
      return;
    }
