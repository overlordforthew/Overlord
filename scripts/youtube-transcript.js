...
async function extractYtdlpSubtitles(videoId) {
  try {
    ...
    // Check for downloaded subtitle file using fs APIs (no shell exec)
    const { readdirSync, readFileSync, unlinkSync } = await import('fs');
    const tmpFiles = readdirSync('/tmp').filter(f => f.startsWith(`yt-${videoId}`) && f.endsWith('.vtt'));

    if (!tmpFiles.length) return null;

    const vttFile = `/tmp/${tmpFiles[0]}`;
    const vttContent = readFileSync(vttFile, 'utf8');
    // Clean up temp files
    readdirSync('/tmp').filter(f => f.startsWith(`yt-${videoId}`)).forEach(f => { try { unlinkSync(`/tmp/${f}`); } catch {} });
