...
USAGE:
  ...
  mem context <query>                   Get formatted context block
  mem ingest <path> [--name N]          Save source to wiki raw/ and show ingest context
  mem lint                              Wiki health-check (orphans, stale, stubs, dead links)
...
Tool events: ${s.total_events} total, ${s.uncompressed_events} uncompressed`);
    ...
    case 'ingest': {
      ...
      if (!filePath) { console.error('Usage: mem ingest <path> [--name name]'); process.exit(1); }
      const nameFlag = args.indexOf('--name');
      const name = nameFlag !== -1 && args[nameFlag + 1] ? args[nameFlag + 1] : filePath.split('/').pop().replace('.md', '');
      try {
        const { readFileSync } = await import('fs');
        const content = readFileSync(filePath, 'utf8');
        const { saveSource, appendLog, getIngestContext, regenerateIndex } = await import('../knowledge-engine.js');
        const savedPath = saveSource(name, content);
        appendLog('ingest', name, `Source saved from ${filePath}. Ready for wiki processing.`);
        console.log(`Source saved: ${savedPath}`);
        console.log(`\nWiki state for ingest context:\n`);
        console.log(getIngestContext());
      } catch (err) {
    ...
    case 'lint': {
      try {
        const { lintWiki } = await import('../knowledge-engine.js');
        const report = lintWiki();
        console.log(`Wiki Health Report`);
        console.log(`${'─'.repeat(40)}`);
        console.log(`Pages: ${report.total_pages}  |  Sources: ${report.total_sources}`);
        console.log(`Status: ${report.healthy ? '✅ Healthy' : '⚠️  Issues found'}\n`);
        if (report.orphans.length) {
          console.log(`Orphan pages (no inbound links): ${report.orphans.length}`);
          for (const o of report.orphans) console.log(`  - ${o.category}/${o.file}.md`);
          console.log();
        }
        if (report.stale.length) {
          console.log(`Stale pages (>30 days): ${report.stale.length}`);
          for (const s of report.stale) console.log(`  - ${s.category}/${s.file}.md (${s.daysSinceUpdate}d)`);
          console.log();
        }
        if (report.stubs.length) {
          console.log(`Stubs (<100 words): ${report.stubs.length}`);
          for (const s of report.stubs) console.log(`  - ${s.category}/${s.file}.md (${s.words} words)`);
          console.log();
        }
        if (report.deadLinks.length) {
          console.log(`Dead links: ${report.deadLinks.length}`);
          for (const d of report.deadLinks) console.log(`  - ${d.from} → ${d.to}`);
          console.log();
        }
        if (report.uningested.length) {
          console.log(`Uningested sources: ${report.uningested.length}`);
          for (const u of report.uningested) console.log(`  - raw/${u}.md`);
          console.log();
        }
