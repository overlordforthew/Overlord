...
const DATA_DIR = existsSync('/app/data')
  ? '/app/data'
  : (existsSync('/root/overlord/data') ? '/root/overlord/data' : path.resolve(ROOT, 'data'));
...
function getCurrentSkills() {
  const skillsDir = existsSync('/app/skills')
    ? '/app/skills'
    : (existsSync('/root/overlord/skills') ? '/root/overlord/skills' : path.join(ROOT, 'skills'));
  try {
...
function analyzeFriction() {
  const frictionPath = existsSync('/app/data/meta-learning/friction.json')
    ...
    : (existsSync('/root/overlord/data/meta-learning/friction.json')
      ? '/root/overlord/data/meta-learning/friction.json'
      : path.join(DATA_DIR, 'meta-learning', 'friction.json'));
...
function summarizeJobRuns(runs, nowMs) {
  ...
  for (const run of runs) {
    const startedAt = Date.parse(run.startedAt || run.finishedAt || 0);
    if (!startedAt || startedAt < weekAgoMs) continue;

    const stats = byJob.get(run.jobId) || {
      jobId: run.jobId,
      label: run.label || run.jobId,
      ok: 0,
      failed: 0,
      skipped: 0,
      total: 0,
...
async function summarizeReliability() {
  ...
  const jobState = await loadJobState(DATA_DIR);
  ...
  const staleJobs = jobs.filter((job) => {
    ...
    const referenceAt = Date.parse(job.lastSuccessAt || job.lastRunAt || 0);
  ...
  const recentRepairTasks = tasks
    ...
    .filter((task) => {
      ...
      return createdAt && (nowMs - createdAt) <= WEEK_MS;
    });

  const failedRepairs = recentRepairTasks
    .filter((task) => task.status === 'blocked' || task.status === 'abandoned')
    ...
    .map((task) => ({
      ...
      verifier: task.verifier?.type || (task.verificationUrl ? 'http' : null),
      verificationEvidence: task.verificationEvidence || null,
    }));

  const retryHeavyRepairs = recentRepairTasks
    .filter((task) => (task.retryCount || 0) > 0)
...
async function generateReport() {
  ...
  // Run all analyses
  const [repos, friction, reliability] = await Promise.all([
    discoverRepos(),
  ...
  // Section 1: Reliability-first scorecard
  lines.push(`RELIABILITY SCORECARD (${reliability.recentRuns} recent runs tracked):`);
  lines.push(`  Jobs tracked: ${reliability.trackedJobs}`);
  lines.push(`  Failing now: ${reliability.failingJobs.length}`);
  lines.push(`  Stale against SLA: ${reliability.staleJobs.length}`);
  lines.push(`  Failed repairs this week: ${reliability.failedRepairs.length}`);
  lines.push('');

  if (reliability.failingJobs.length > 0) {
    lines.push('FAILING JOBS NOW:');
    for (const job of reliability.failingJobs.slice(0, 5)) {
      lines.push(`  ${job.label || job.id}: ${job.lastFailure || 'failed recently'}`);
    }
    lines.push('');
  }

  if (reliability.staleJobs.length > 0) {
    lines.push('STALE JOBS:');
    for (const job of reliability.staleJobs.slice(0, 5)) {
      lines.push(`  ${job.label || job.id}: last success ${job.lastSuccessAt || 'never'} | SLA ${job.freshnessSlaMinutes}m`);
    }
    lines.push('');
  }
  ...
  if (reliability.failedRepairs.length > 0) {
    lines.push('FAILED REPAIRS (7 days):');
    for (const repair of reliability.failedRepairs) {
      const verifier = repair.verifier ? ` | verifier: ${repair.verifier}` : '';
      lines.push(`  ${repair.title} (${repair.status}, retries ${repair.retryCount})${verifier}`);
      if (repair.verificationEvidence) {
        lines.push(`    Evidence: ${repair.verificationEvidence.substring(0, 140)}`);
      }
    }
    lines.push('');
  }

  // Section 2: GitHub Discoveries with deep analysis
  ...
  if (reliability.failedRepairs.length > 0 && recNum <= 3) {
    const top = reliability.failedRepairs[0];
    lines.push(`${recNum}. REVIEW FAILED REPAIR`);
    lines.push(`   ${top.title} stalled at ${top.status} after ${top.retryCount} retry attempt(s).`);
    recNum++;
  }

  if (highPriorityGaps.length > 0 && recNum <= 3) {
  ...
  if (analyses.length > 0 && analyses[0].analysis && recNum <= 3) {
  ...
  if (friction.patterns.length > 0 && recNum <= 3) {
  ...
  lines.push('');
  lines.push(`${duration}s research | ${reliability.trackedJobs} jobs tracked | ${reliability.failedRepairs.length} failed repairs | ${analyses.length} repos deep-analyzed`);
  ...
  return {
    ...
    metadata: {
      ...
      trackedJobs: reliability.trackedJobs,
