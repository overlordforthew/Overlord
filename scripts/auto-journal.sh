...
for LOG_FILE in "$LOG_DIR"/*.jsonl; do
    ...
    TODAY_MSGS=$(grep -c "\"t\":\"${DATE}" "$LOG_FILE" 2>/dev/null || true)
    TODAY_MSGS=${TODAY_MSGS:-0}; TODAY_MSGS=$(echo "$TODAY_MSGS" | tr -d '[:space:]')
    TODAY_BOT=$(grep "\"t\":\"${DATE}" "$LOG_FILE" 2>/dev/null | grep -c '"role":"bot"' 2>/dev/null || true)
    TODAY_BOT=${TODAY_BOT:-0}; TODAY_BOT=$(echo "$TODAY_BOT" | tr -d '[:space:]')
