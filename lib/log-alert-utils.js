...
// Overlord retries `docker compose`/`docker build` failures internally; first-try failure
// is noise when the retry succeeds. Real impact (container didn't come up) is still caught
// by the docker event watcher, so these pino level:50 self-reports shouldn't page.
const TRANSIENT_BUILD_COMMAND_MESSAGE = /^Command failed:[^\n]*\bdocker(?:[- ]compose| build)\b/i;
...
function shouldIgnoreLogLine(line, ignorePatterns) {
  ...
  try {
    ...
    if (parsed.level === 50 && typeof parsed?.err?.message === 'string' &&
        TRANSIENT_BUILD_COMMAND_MESSAGE.test(parsed.err.message)) {
      return true;
    }
  } catch {}
